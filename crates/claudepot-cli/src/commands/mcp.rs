...
/// Codex how to use the Claudepot MCP memory tools. Stable across
/// regenerations so users can `@include` it from CLAUDE.md /
/// AGENTS.md without their narrative documents drifting.
///
/// The version stamp at the top lets us update the snippet over
/// time without leaving the user puzzled about why their copy
/// disagrees with a future installer's output — a regen overwrites
/// in place, and they `@include` from a fixed path.
pub const SNIPPET_VERSION: &str = "1";

pub fn snippet_body() -> String {
    format!(
        r#"<!-- claudepot-mcp-instructions v{version} — managed by `claudepot mcp install-snippet` -->
...
install-snippet`; running it again refreshes the content. Your
CLAUDE.md / AGENTS.md should `@include` it once and never
duplicate it inline.
"#,
...
mod snippet_tests {
    ...
    fn snippet_has_version_header() {
        ...
        assert!(
            body.contains(&format!("claudepot-mcp-instructions v{}", SNIPPET_VERSION)),
