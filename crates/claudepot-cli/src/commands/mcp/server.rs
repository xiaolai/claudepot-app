//! `memory-server` verb — the stdio MCP server itself: tool
//! input/output structs, the `tool_router` impl, and the emission
//! helpers (redaction, error envelopes).
//!
//! Sub-module of `commands/mcp.rs`; see that file's header for the
//! verb-group layout rationale and the tool list.
