...
///
/// `MigrateError` is lifted with its identity intact (`ErrorDto::from`)
/// rather than stringified — the pane branches on "wrong passphrase"
/// vs "the signature did not verify", and `e.to_string()` threw that
/// away. See `crate::dto_error`.
#[tauri::command]
pub async fn migrate_inspect(args: InspectArgsDto) -> Result<ImportPlanDto, ErrorDto> {
    let bundle = PathBuf::from(args.bundle_path);
    if bundle.extension().map(|e| e == "age").unwrap_or(false) {
        let pwd = promote_secret(args.passphrase).ok_or_else(|| {
            ErrorDto::new(
                codes::MIGRATE_PASSPHRASE_REQUIRED,
                "encrypted bundle requires passphrase",
            )
        })?;
        let m = migrate::inspect_encrypted(&bundle, &pwd)?;
        return Ok(m.into());
    }
    let m = migrate::inspect(&bundle)?;
    Ok(m.into())
}
...
#[tauri::command]
pub async fn migrate_export(args: ExportArgsDto) -> Result<ExportReceiptDto, ErrorDto> {
    ...
    let account_stubs = if args.include_claudepot_state {
        ...
        // `AccountStore::open` returns a bare `rusqlite::Error`, so this
        // shares the store-open code the account surfaces already mint.
        let store = AccountStore::open(&data_dir.join("accounts.db"))
            .map_err(|e| ErrorDto::detail(codes::ACCOUNTS_STORE_OPEN_FAILED, e))?;
        Some(migrate_state::account_stubs_from_store(&store)?)
    } else {
        None
    };
...
/// Import a bundle.
#[tauri::command]
pub async fn migrate_import(args: ImportArgsDto) -> Result<ImportReceiptDto, ErrorDto> {
    let config_dir = paths::claude_config_dir();
    let mode = match args.mode.as_str() {
        ...
        other => {
            return Err(ErrorDto::with_params(
                codes::MIGRATE_UNKNOWN_CONFLICT_MODE,
                json!({ "mode": other }),
    ...
    let prefer = match args.prefer.as_deref() {
        ...
        Some(other) => {
            return Err(ErrorDto::with_params(
                codes::MIGRATE_UNKNOWN_MERGE_PREFERENCE,
                json!({ "prefer": other }),
                format!("unknown prefer: {other}"),
            ))
        }
        None => None,
    };
...
#[tauri::command]
pub async fn migrate_undo() -> Result<UndoReceiptDto, ErrorDto> {
    let r = migrate::import_undo()?;
    Ok(r.into())
}
