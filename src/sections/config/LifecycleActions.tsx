...
export function LifecycleActions({
  ...
  // Loading / not-yet-classified: render nothing rather than a flash
  // of action buttons.
  ...
  if (classification.refused) {
    return (
      ...
      >
        <Glyph g={NF.info} />
        {refusedShort(classification.refused)}
      </div>
    );
  }

  const trackable = classification.trackable;
  if (!trackable) return null; // Defensive: no refused, no trackable

  const isDisabled = classification.already_disabled;
  ...
  const onDisable = useCallback(async () => {
    ...
    try {
      ...
      // Conflict → offer the suffix retry.
      if (/already exists/i.test(msg)) {
        if (window.confirm(`A disabled ${trackable.kind} with that name already exists. Add a "-N" suffix?`)) {
          try {
            const rec = await api.artifactDisable(
              trackable.scope_root,
              trackable.kind,
            ...
            pushToast(
              "error",
              `Disable failed: ${err2 instanceof Error ? err2.message : String(err2)}`,
            );
          }
        }
      } else {
  ...
  const onTrash = useCallback(async () => {
    if (
      !window.confirm(
        `Move ${trackable.kind} "${trackable.relative_path}" to trash? You can restore it from Settings → Cleanup for ~30 days.`,
      )
    ) {
      return;
    }
