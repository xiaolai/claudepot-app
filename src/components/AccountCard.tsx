...
export function AccountCard({
  account: a, desktopAvailable, busyKeys, anyBusy, expanded, onToggleExpand,
  ...
  const desktopDisabled = selfBusy || a.is_desktop_active || !desktopAvailable || !a.has_desktop_profile;
  const hint = !a.is_desktop_active && desktopDisabled && !selfBusy
    ? getDisabledHint(a, desktopAvailable) : null;
  ...
  return (
    <article className={`account ${a.is_cli_active ? "cli-active" : ""} ${a.is_desktop_active ? "desktop-active" : ""} ${expanded ? "expanded" : ""}`}
      ...
      {expanded && <AccountDetail account={a} />}
    </article>
  );
}
